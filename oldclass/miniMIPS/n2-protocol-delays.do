### Tcl test bench for the n2 protocol suite.
###
...
while { 1 } {
    ...
    ### Break condition
    if { $cycle_time != 0 } {
	if { [gtTime $Now [addTime $last_lr_up_time [mulTime $cycle_time 3ns]]] } {
	    #echo "Breaking loop - three times max cycle without another req"
	    #echo "$Now > [addTime $last_lr_up_time [mulTime $cycle_time 3ns]]"
	    break
	}
    }
